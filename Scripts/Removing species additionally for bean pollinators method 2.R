...
# ================== Script if theta is less than zero ========================
...
for(i in bean_species_list){  # Open loop
  ...
  id <- order(bean_growth_rate_mean1$year)
  
  # Cacluate area under the curve i.e. area between line and zero
  
  AUC <-
    sum(
      diff(bean_growth_rate_mean1$year[id]) * 
        rollmean(bean_growth_rate_mean1$mean.growth.rate[id], 2)
    )
  ...
  y2 <- rep(-0.1, 31)
  df<- cbind(bean_growth_rate_mean1$year,y2)
  df <- as.data.frame(df)
  ...
  AUC2 <- sum(diff(bean_growth_rate_mean1$year[id])*rollmean(y2[id],2))
...
# ================== Script if theta is greater than zero =====================
...
bean_growth_rate_mean2 <- bean_growth_rate_mean
...
for(i in bean_species_list){  # Open loop
  ...
  bean_growth_rate_mean$mean.growth.rate[bean_growth_rate_mean$mean.growth.rate < 0] <- 0
  ...
  bean_growth_rate_mean2$mean.growth.rate[bean_growth_rate_mean2$mean.growth.rate > 0] <- 0
  
  
  # Create plot of mean yearly growth rates across all species using subset of data
  
  bean_mean_growth_rate_plot <- ggplot(
    bean_growth_rate_mean2, aes(
      x = year,
      y = mean.growth.rate)) + 
    geom_hline(yintercept = 0) +
    coord_cartesian(xlim = c(1985, 2015), ylim = c(-1.5, 2)) +
  ...
  id <- order(bean_growth_rate_mean2$year)
  
  # Cacluate area under the curve i.e. area between line and zero
  
  AUC2 <-
    sum(
      diff(bean_growth_rate_mean2$year[id]) * 
        rollmean(bean_growth_rate_mean2$mean.growth.rate[id], 2)
    )
  ...
  AUC3 <- sum(diff(bean_growth_rate_mean$year[id])*rollmean(y2[id],2))
  
  
  # Subtract area between theta and zero from original calculation of AUC to get only area
  # between the line and theta.
  
  deficit_value <- AUC2 - AUC3 + AUC
